...
pipeline {
    ...
    stages {
            ...
            steps {
                script {
                    ...
                    bat """
                        ...
                        for /f "tokens=*" %%i in (\'aws eks create-token --cluster-name fastapi-eks --region %AWS_REGION% --query "status.token" --output text\') do set JOIN_CMD=%%i
                        for /f "tokens=*" %%i in (\'aws eks describe-cluster --name fastapi-eks --region %AWS_REGION% --query "cluster.endpoint" --output text\') do set ENDPOINT=%%i
                        for /f "tokens=*" %%i in (\'aws eks describe-cluster --name fastapi-eks --region %AWS_REGION% --query "cluster.certificateAuthority.data" --output text ^| base64 -d ^| sha256sum ^| awk "{print \$1}"\') do set HASH=%%i
        ...
        stage('Deploy to EKS') {
            steps {
                bat '''
                    aws eks --region %AWS_REGION% update-kubeconfig --name fastapi-eks || exit /b 1
